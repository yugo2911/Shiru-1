<script>
  ...
  function close() {
    if (overlay.includes("trailer"))
      overlay = overlay.filter((item) => item !== "trailer");
    trailer.set(false);
  }
  ...
  $: if ($trailer && !overlay.includes("trailer"))
    overlay = [...overlay, "trailer"];
...
<button
  class="btn btn-icon bg-dark-light text-white d-none align-items-center justify-content-center shadow-none border-0 mr-10"
  class:d-flex={!$hide}
  data-toggle="tooltip"
  data-placement="top"
  data-target-breakpoint="md"
  data-title="Watch Trailer"
  use:click={() => trailer.set(!$trailer)}
>
  <TvMinimalPlay size="2rem" />
</button>
<SoftModal
  class="pointer-events-none w-full scrollbar-none align-items-center mb-30"
  css={`top-0 left-0 position-fixed`}
  bind:showModal={$trailer}
  shouldRender={true}
  {close}
  id="viewTrailer"
>
  <div
    class="pointer-events-auto d-flex align-items-center rounded-top-5 w-full wm-calc bg-dark h-40"
  >
    <span
      class="title ml-20 font-weight-very-bold text-muted select-all mr-20 font-scale-18"
      >{anilistClient.title(staticMedia)}</span
    >
    <button
      type="button"
      class="btn btn-square bg-transparent shadow-none border-0 d-flex align-items-center justify-content-center ml-auto mr-5"
      use:click={close}><X size="1.7rem" strokeWidth="3" /></button
    >
  </div>
  <div
    class="pointer-events-auto ratio-16-9 position-relative w-full wm-calc overflow-hidden rounded-bottom-5"
  >
    ...
    {#await (staticMedia.trailer?.id && staticMedia) || episodesList.getMedia(staticMedia.idMal) then trailerUrl}
      {@const trailerId =
        trailerUrl?.trailer?.id || trailerUrl?.data?.trailer?.youtube_id}
      {#if trailerId}
        {show()}
        {#if $trailer}
          <div
            class="pointer-events-auto ratio-16-9 position-relative w-full wm-calc"
          >
            <SmartImage
              class="ratio-16-9 img-cover w-full h-full rounded-bottom-6"
              images={[
                ...(trailerId
                  ? [
                      `https://i.ytimg.com/vi/${trailerId}/maxresdefault.jpg`,
                      `https://i.ytimg.com/vi/${trailerId}/hqdefault.jpg`,
                    ]
                  : []),
